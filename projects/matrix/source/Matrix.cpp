...
: mSize(std::exchange(another.mSize, {0, 0}))
, mData(std::exchange(another.mData, nullptr))
{
}
...
Matrix operator*(const Matrix& lhs, const Matrix& rhs) {
    ...
    const unsigned rowsNumber = lhs.mSize.rows;
    const unsigned columnsNumber = rhs.mSize.columns;
    const unsigned internalSize = lhs.mSize.columns;
